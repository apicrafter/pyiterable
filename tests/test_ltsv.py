...
def test_ltsv_write():
    ...
    try:
        ...
        writer.write({"time": "2023-01-01T00:00:00Z", "host": "example.com", "status": "200", "method": "GET"})
        writer.write({"time": "2023-01-01T00:01:00Z", "host": "example.com", "status": "404", "path": "/notfound"})
        writer.close()
...
def test_ltsv_none_values():
    ...
    try:
        writer = LTSVIterable(tmp_path, mode="w")
        writer.write({"key1": "value1", "key2": None, "key3": "value3"})
        writer.close()
...
def test_ltsv_numeric_values():
    ...
    try:
        writer = LTSVIterable(tmp_path, mode="w")
        writer.write({"status": 200, "count": 42, "ratio": 0.95})
        writer.close()
