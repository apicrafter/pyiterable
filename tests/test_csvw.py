...
def setup_module():
    ...
    if not os.path.exists(METADATA_FILE):
        ...
        metadata = {
            ...
            "tableSchema": {
                "columns": [
                    {"name": "id", "titles": "id", "datatype": {"base": "string"}},
                    {"name": "name", "titles": "name", "datatype": {"base": "string"}},
                ]
            },
        }
...
class TestCSVW:
    ...
    def test_write_read(self):
        iterable = CSVWIterable("testdata/2cols6rows_test.csvw", mode="w", options={"keys": ["id", "name"]})
        # Write records
        for record in FIXTURES:
