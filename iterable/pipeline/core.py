...
def pipeline(source: BaseIterable, destination: BaseIterable, process_func: Callable[[dict, dict], dict], trigger_func:Callable[[dict, dict], None] = None, trigger_on:int=1000, final_func: Callable[[dict, dict], None] = None, reset_iterables: bool = True, skip_nulls: bool = True, start_state:dict=None, debug:bool = False, batch_size:int=1000):
    """Wrapper over Pipeline class to simplify data processing pipelines execution"""
    if start_state is None:
        start_state = {}
    runner = Pipeline(source=source, destination=destination, process_func=process_func, trigger_func=trigger_func, trigger_on=trigger_on, final_func=final_func, reset_iterables=reset_iterables, skip_nulls=skip_nulls, start_state=start_state, batch_size=batch_size)
    runner.run(debug)


class Pipeline:
    """Data processing pipeline that read data and process it"""
    def __init__(self, source: BaseIterable, destination: BaseIterable = None, process_func: Callable[[dict, dict], dict]=None, trigger_func:Callable[[dict, dict], None] = None, trigger_on:int=1000, final_func: Callable[[dict, dict], None] = None, reset_iterables: bool = True, skip_nulls: bool = True, start_state:dict=None, batch_size:int=1000):
        if start_state is None:
            start_state = {}
        self.source = source
    ...
    def run(self, debug:bool = False):
        ...
        batch: list[dict] = []
        can_bulk_write = self.destination is not None and hasattr(self.destination, "write_bulk") and self.batch_size and self.batch_size > 1
